...
// samplePolyCBDGeneric draws a ringElement from the Dη distribution given the
// pre-computed PRF output B (length 64*η), according to FIPS 203, Algorithm 8.
// SamplePolyCBD draws 2η bits per coefficient: the first η bits sum to "a",
// the next η bits sum to "b", and the coefficient is fieldSub(a, b).
func samplePolyCBDGeneric(B []byte, η byte) ringElement {
	var f ringElement
	switch η {
	case 2:
		for i := 0; i < n; i += 2 {
...
func decodeAndDecompressU10Generic(dst []ringElement, c []byte) {
	for i := range dst {
		off := encodingSize10 * i
		b := (*[encodingSize10]byte)(c[off : off+encodingSize10])
		dst[i] = ringDecodeAndDecompress10(b)
	}
}

func decodeAndDecompressU11Generic(dst []ringElement, c []byte) {
	for i := range dst {
		off := encodingSize11 * i
		b := (*[encodingSize11]byte)(c[off : off+encodingSize11])
		dst[i] = ringDecodeAndDecompress11(b)
	}
}
