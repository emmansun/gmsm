...
func (sk *PrivateKey65) ensureT1() {
	...
	sk.t1Once.Do(func() {
		...
		for i := range nttT {
			nttMul(&nttT[i], &s1NTT[0], &A[i*l65])
			for j := 1; j < l65; j++ {
				nttMulAcc(&nttT[i], &s1NTT[j], &A[i*l65+j])
			}
		}
...
func dsaKeyGen65(sk *Key65, xi *[32]byte) {
	...
	for r := range byte(k65) {
		for s := byte(0); s < l65; s++ {
			A[r*l65+s] = rejNTTPoly(G128, rho, s, r)
		}
	}
...
func (pk *PublicKey65) verifyInternal(sig, mu []byte) bool {
	...
	pk.ensureNTT()
	var zNTTMulA [k65]nttElement
	for i := range k65 {
		nttMul(&zNTTMulA[i], &zNTT[0], &pk.a[i*l65])
		for j := 1; j < l65; j++ {
			nttMulAcc(&zNTTMulA[i], &zNTT[j], &pk.a[i*l65+j])
		}
