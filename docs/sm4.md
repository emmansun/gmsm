...
* SIMD并行处理：借助CPU的GFNI指令，部分新AMD64 CPU架构支持该指令，本软件库已实现[SM4 with GFNI](https://github.com/emmansun/gmsm/wiki/SM4-with-GFNI)
* SIMD并行处理：位切片(bitslicing)，[参考实现](https://github.com/emmansun/sm4bs)

当然，这些与有CPU指令支持的AES算法相比，性能差距依然偏大，要是工作模式不支持并行，差距就更巨大了。

### 混合方式
从**v0.25.0**开始，AMD64/ARM64 支持AES-NI的CPU架构下，**默认会使用混合方式**，即```cipher.Block```的方法会用纯Go语言实现，而对于可以并行的加解密模式，则还是会尽量采用AES-NI和SIMD并行处理。您可以通过环境变量```FORCE_SM4BLOCK_AESNI=1```来强制```cipher.Block```单块加解密也使用汇编实现（和v0.25.0之前版本的行为一样）。请参考[SM4: 单block的性能问题](https://github.com/emmansun/gmsm/discussions/172)。

设置```FORCE_SM4BLOCK_AESNI=1```时，在支持GFNI的AMD64 CPU上（可通过```DISABLE_GFNI=1```禁用），单块加解密会优先选用GFNI实现；否则退回到AES-NI实现。三种单块实现的性能参考（Intel i7-13700，Windows AMD64）：

| 实现 | 耗时 | 吞吐量 | 说明 |
|------|------|--------|------|
| 纯Go查表（默认） | ~116 ns | ~138 MB/s | 最快，非恒定时间 |
| GFNI（`FORCE_SM4BLOCK_AESNI=1`） | ~143 ns | ~112 MB/s | 恒定时间，比AES-NI快约28% |
| AES-NI（`FORCE_SM4BLOCK_AESNI=1` + `DISABLE_GFNI=1`） | ~199 ns | ~80 MB/s | 恒定时间，旧基准 |
