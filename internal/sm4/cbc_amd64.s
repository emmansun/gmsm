...
// GFNI CBC decrypt path
// Register plan:
//   Y3  = GFNI_POST (constant, survives TRANSPOSE which only touches data + tmp0/tmp1)
//   Y1  = GFNI_PRE (loaded after each input TRANSPOSE, clobbered by output TRANSPOSE - OK)
//   Y15 = RESERVED for X15 (last ciphertext block = new IV)
//   Y13 = BYTE_FLIP_MASK
//   Y2  = BSWAP_MASK

gfniCbcStart:
	VMOVDQU ·gfni_post_matrix(SB), NIBBLE_MASK  // Y3 = post matrix (constant)
	VBROADCASTI128 ·flip_mask(SB), BYTE_FLIP_MASK
	VBROADCASTI128 ·bswap_mask(SB), BSWAP_MASK

	VMOVDQU -16(DX), X15  // Save last ciphertext block as new IV

gfniCbc_16blocks:
	CMPQ DI, $256
	JLE gfniCbcSm4Octets
	SUBQ $256, DI
	LEAQ -256(DX), DX
	LEAQ -256(BX), BX
	...
	// Load pre-matrix after TRANSPOSE (which clobbers Y0, Y1)
	VMOVDQU ·gfni_pre_matrix(SB), XDWTMP1  // Y1 = pre matrix

	GFNI_SM4_16BLOCKS(AX, XDWTMP0, XDWORD, YDWORD, XDWTMP1, NIBBLE_MASK, XDWORD0, XDWORD1, XDWORD2, XDWORD3, XDWORD4, XDWORD5, XDWORD6, XDWORD7)
	...
	JMP gfniCbc_16blocks

gfniCbcSm4Octets:
	CMPQ DI, $128
	JLE gfniCbcSm4Nibbles
	SUBQ $128, DI
	LEAQ -128(DX), DX
	LEAQ -128(BX), BX
	...
	// Load pre-matrix after TRANSPOSE
	VMOVDQU ·gfni_pre_matrix(SB), XDWTMP1  // Y1 = pre matrix

	GFNI_SM4_8BLOCKS(AX, XDWORD, YDWORD, XDWTMP0, XDWTMP1, NIBBLE_MASK, XDWORD0, XDWORD1, XDWORD2, XDWORD3)
	...
	JMP gfniCbcSm4Octets

gfniCbcSm4Nibbles:
	// For remaining <= 64 bytes, reload NIBBLE_MASK and use AVX2 tail path
	VBROADCASTI128 ·nibble_mask(SB), NIBBLE_MASK
	JMP avx2CbcSm4Nibbles
