...
func expectUnhandledCriticalExtension(t *testing.T, err error) {
	if _, ok := err.(x509.UnhandledCriticalExtension); !ok {
...
func generateCert(cn string, isCA bool, issuer *x509.Certificate, issuerKey crypto.PrivateKey) (*Certificate, crypto.PrivateKey, error) {
