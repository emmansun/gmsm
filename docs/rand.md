...
`rand.Read()` 的性能主要取决于：
- **首次调用**：需要收集熵和初始化 DRBG（约 4-5ms），包含 KAT 自检
- **后续调用**：直接从 DRBG 生成，每次额外获取 16 字节 OS 附加输入
- **重播种时**：需要重新收集三重熵（约 4-5ms）

> **Seed() 耗时构成**（13th Gen Intel Core i7-13700，Windows，~4.2ms/次）：
> - CPU 抖动源：~1ms（1024 次内存访问采样）
> - 哈希循环源：~3ms（1024 次 × 16 次 SM3 + Gosched）
> - OS 熵 + 池操作：<0.1ms
