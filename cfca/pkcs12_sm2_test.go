...
func TestParseSM2WithInvalidPwd(t *testing.T) {
	...
	_, _, err := ParseSM2(password, der)
	if err == nil || err.Error() != "cfca: failed to decrypt by SM4-CBC, please ensure the password is correct: padding: invalid padding" {
		t.Fatal("cfca: failed to decrypt by SM4-CBC, please ensure the password is correct: padding: invalid padding")
	}
}
