...
| 熵源 | 实现方式 | 健康测试 | 熵估计 |
|------|---------|---------|--------|
| OS 系统随机数 | `crypto/rand.Reader` | 无（OS 保证质量） | 256 位/32 字节 |
| CPU 抖动 | 高精度计时 + 内存访问噪声 | 重复计数 + 自适应比例 | 1024 位/1024 样本 |
| 运行时噪声 | `runtime.Gosched()` 调度抖动 + 内存分配 | 重复计数 + 自适应比例 | 1024 位/1024 样本 |
